...
openssl x509 -req -in aggr-server.csr -CA root-ca.crt -CAkey root-ca.key -CAcreateserial -out aggr-server.crt -days 365 -sha384 -extfile aggr-server.cnf
...
openssl x509 -req -in na-server.csr -CA root-ca.crt -CAkey root-ca.key -CAcreateserial -out na-server.crt -days 365 -sha384 -extfile na-server.cnf
